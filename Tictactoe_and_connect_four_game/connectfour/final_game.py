...
turn = random.randint(User, Computer)
...
while not game_over:
            ...
            if turn == User:
                ...
                if is_valid_location(board, col):
                    ...
                    if winning_move(board, PLAYER_PIECE):
                        label = myfont.render("User Win", 1, Orange)
    ...
    if turn == Computer and not game_over:				
        col = int(loaded_model.predict(pd.DataFrame(np.flip(board,axis=0).T.reshape(1,-1))))
        print(col)
        ...
        if is_valid_location(board, col):
            ...
            if winning_move(board, AI_PIECE):
                label = myfont.render("Computer Win", 1, Green)
                screen.blit(label, (40,10))
                game_over = True
